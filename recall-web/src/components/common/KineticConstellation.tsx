    ...
    <div ref={containerRef} className="relative h-full w-full min-h-[460px] overflow-hidden rounded-2xl border border-[#222a3a] bg-[#0c0f15]/80 backdrop-blur-sm">
      ...
      {activeNode && (
        <div className="pointer-events-none absolute bottom-4 left-4 right-4 sm:right-auto sm:max-w-xs rounded-xl border border-slate-700 bg-[#121722]/95 p-4 shadow-2xl backdrop-blur-md transition-all">
          ...
          <p className="text-xs text-slate-400 mb-2">Core Neural Mastery Pathway</p>
          <div className="flex flex-wrap gap-1.5">
            {activeNode.skills.map((skill, i) => (
              <span key={i} className="rounded-md border border-slate-800 bg-[#19202e] px-2 py-0.5 text-[11px] text-slate-300 font-mono">
                {skill}
              </span>
            ))}
          </div>
      ...
      <div className="absolute top-4 right-4 flex items-center gap-2 rounded-full border border-slate-800 bg-[#10141d]/80 px-3 py-1 text-[11px] text-slate-400 backdrop-blur-sm">
