...
export const ScrollStackItem = ({ children, itemClassName = '' }: any) => (
  <div
    className={`scroll-stack-card relative w-full h-[60vh] min-h-[400px] my-8 p-10 md:p-16 rounded-[40px] shadow-[0_20px_50px_rgba(0,0,0,0.1)] box-border origin-top will-change-transform bg-white border border-slate-100 ${itemClassName}`.trim()}
...
export const ScrollStack = ({
  ...
  }, [itemDistance, itemScale, itemStackDistance, stackPosition, scaleEndPosition, baseScale, rotationAmount, useWindowScroll, setupLenis, updateCardTransforms]);

  return (
    <div className={`relative w-full ${className}`} ref={scrollerRef}>
      <div className="scroll-stack-inner pt-[10vh] pb-[30rem] w-full max-w-5xl mx-auto px-4">
        {children}
