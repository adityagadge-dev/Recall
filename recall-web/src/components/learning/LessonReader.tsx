    ...
    <div id="lesson-reader-container" className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 pb-4">
        <div>
          ...
          <h2 className="text-sm font-semibold text-slate-600 mt-0.5">{module.title}</h2>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className={`flex items-center gap-1.5 rounded-xl border px-3 py-1.5 text-xs font-medium transition ${
              isBookmarked
                ...
                : 'border-slate-200 bg-slate-50 text-slate-600 hover:text-[#0F172A]'
            }`}
          >
          ...
          <button
            ...
            className="flex items-center gap-1.5 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 text-xs font-medium text-slate-600 hover:text-[#0F172A] transition"
          >
      ...
      <div className="rounded-2xl border border-teal-50 bg-gradient-to-r from-cyan-950/20 to-transparent p-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div className="flex items-center gap-2.5">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-bold text-[#0F172A]">Google ADK Learning Agent</span>
              ...
              <p className="text-[11px] text-slate-600">Personalized on Creator-verified research grounding</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <button
              ...
              className="flex items-center gap-1.5 rounded-lg border border-teal-50 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold bg-teal-500 hover:bg-teal-50 transition disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={aiState !== 'idle'}
              className="flex items-center gap-1.5 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-600 hover:text-[#0F172A] transition disabled:opacity-50"
            >
      ...
      {/* Lesson Header */}
      <div>
        <div className="flex items-center gap-2 mb-2 text-xs font-mono text-slate-600">
          <span>{lesson.durationMinutes} MIN READ</span>
          <span>•</span>
          <span className="text-[#0F766E] font-bold">+{lesson.xpReward} XP REWARD</span>
        </div>
        <h1 className="text-2xl sm:text-3xl font-extrabold text-[#0F172A] tracking-tight">{lesson.title}</h1>
        <p className="text-sm text-slate-600 mt-1">{lesson.subtitle}</p>
      </div>
      ...
      {notesOpen && (
        ...
        >
          <div className="flex items-center justify-between">
            <h4 className="text-xs font-bold uppercase tracking-wider text-[#0F172A]">Your Personal Notes</h4>
            <span className="text-[10px] text-slate-600">Auto-synced with learning profile</span>
          </div>
          <textarea
            rows={3}
            value={userNote}
            ...
            className="w-full rounded-xl border border-slate-200 bg-white p-3 text-xs text-slate-700 placeholder-slate-500 focus:border-[#0F766E] focus:outline-none resize-none"
          />
      ...
      <div className="space-y-6 text-slate-700 leading-relaxed">
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 sm:p-6 space-y-2">
          <h3 className="text-sm font-bold uppercase tracking-wider text-slate-600">1. Conceptual Primer</h3>
        ...
        {/* Key Mechanism Block */}
        <div className="rounded-2xl border border-teal-50 bg-slate-50 p-5 sm:p-6 space-y-3">
          ...
          <p className="text-sm sm:text-base font-medium text-[#0F172A] leading-relaxed">
        ...
        {lesson.content.interactiveExample && (
          <div className="rounded-2xl border border-slate-200 bg-white p-5 sm:p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h4 className="text-sm font-bold text-[#0F172A]">{lesson.content.interactiveExample.title}</h4>
              <span className="text-[10px] font-mono text-[#0F766E] bg-teal-50 px-2 py-0.5 rounded border border-teal-50">
                Interactive Model
              </span>
            </div>
            <p className="text-xs text-slate-600">{lesson.content.interactiveExample.context}</p>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2">
              {lesson.content.interactiveExample.breakdown.map((item, idx) => (
                <div key={idx} className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 space-y-1">
                  ...
                  <p className="text-[11px] text-slate-600 leading-snug">{item.note}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 sm:p-6 space-y-3">
        ...
        {lesson.supportingResources.length > 0 && (
          <div className="rounded-xl border border-slate-200 bg-white p-4 text-xs space-y-2">
            <span className="font-bold uppercase tracking-wider text-slate-600 text-[10px]">Verified Knowledge Grounding:</span>
      ...
      <div className="rounded-2xl border border-teal-50 bg-slate-50 p-6 text-center space-y-4">
        <div className="flex flex-col items-center">
          ...
          <h3 className="text-lg font-bold text-[#0F172A]">Lesson Completed?</h3>
