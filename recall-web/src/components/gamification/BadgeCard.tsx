  ...
  return (
    <div
      ...
      className={`group relative flex flex-col justify-between rounded-2xl border p-4 transition-all ${
        isUnlocked
          ? 'border-slate-200 bg-white hover:border-teal-200 hover:shadow-md cursor-pointer'
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <div className={`flex h-12 w-12 items-center justify-center rounded-xl border ${tierBorders[badge.tier] || tierBorders.bronze}`}>
            {isUnlocked ? <IconComponent className="h-6 w-6" /> : <Lock className="h-5 w-5 text-slate-500" />}
          </div>
        ...
        <h4 className="text-sm font-bold text-[#0F172A] mb-1 group-hover:text-teal-600 transition">
          {badge.title}
        </h4>
...
export const AchievementCard: React.FC<{
  ...
  return (
    <div className="flex items-center justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 transition hover:border-teal-200 hover:shadow-md">
      <div className="flex items-center gap-3.5 min-w-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h4 className="text-sm font-bold text-[#0F172A] truncate">{achievement.title}</h4>
            {achievement.isCompleted && (
  ...
  return (
    <div className={`relative flex items-center justify-between gap-4 rounded-xl border p-3.5 transition ${
      ...
      <div className="flex items-center gap-3 min-w-0">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2">
            <h5 className="text-xs font-bold text-[#0F172A] truncate">{challenge.title}</h5>
            <span className="rounded bg-[#1a212f] px-1.5 py-0.5 text-[9px] font-mono text-slate-500 uppercase">
              {challenge.type}
            </span>
          </div>
          <p className="text-[11px] text-slate-500 truncate">{challenge.description}</p>
        </div>
      </div>
