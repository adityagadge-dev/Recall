  ...
  return (
    <section className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16 overflow-hidden">
      <div className="text-center max-w-2xl mx-auto mb-16">
        <h2 className="text-3xl font-bold text-[#25364A] mb-4">Four skills everyone should know.</h2>
      ...
      >
        <div className="absolute left-4 sm:left-12 flex flex-col gap-4 z-20 hidden md:flex">
          <button 
            onClick={handlePrev}
            className="w-12 h-12 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center text-slate-500 hover:text-[#25364A] hover:border-slate-300 transition-all hover:-translate-y-1 active:scale-95"
        ...
        <div className="relative w-full max-w-2xl h-full flex items-center justify-center perspective-[1000px]">
          {SUBJECTS.map((subject, index) => {
            ...
            return (
              ...
              >
                {isActive ? (
                  <div className="bg-[#FFFDF8] border border-slate-200 shadow-2xl shadow-[#25364A]/5 rounded-[2rem] overflow-hidden flex flex-col-reverse sm:flex-row group transition-colors">
                    <div className="p-8 sm:p-10 flex-1 flex flex-col justify-center">
                      ...
                      <h3 className="text-2xl sm:text-3xl font-bold text-[#25364A] mb-4">
                        {subject.name}
                      </h3>
                      ...
                      <div>
                        <Link 
                          ...
                          className="inline-flex items-center gap-2 bg-[#F7C928] text-[#25364A] px-6 py-3 rounded-full font-bold shadow-sm hover:bg-[#E5B81F] hover:-translate-y-0.5 transition-all group/btn"
                        >
                    ...
                    <div className="block h-32 sm:h-auto sm:w-2/5 relative overflow-hidden bg-slate-100 shrink-0">
                      <motion.img 
                        src={subject.image}
                        alt={subject.name}
                  ...
                  <div 
                    ...
                    className="max-w-sm mx-auto bg-white/80 backdrop-blur-sm border border-slate-200/60 shadow-lg rounded-3xl p-6 cursor-pointer hover:bg-white hover:border-slate-300 transition-colors flex items-center gap-6 group"
                  >
                    <div className={`w-12 h-12 rounded-xl ${subject.bg} ${subject.color} flex items-center justify-center shrink-0`}>
                      <subject.icon className="w-6 h-6" />
                    </div>
                    <div>
                      <h4 className="text-lg font-bold text-[#25364A] mb-1 group-hover:text-[#F7C928] transition-colors">{subject.name}</h4>
        ...
        <div className="absolute -bottom-8 left-1/2 -translate-x-1/2 flex gap-4 md:hidden z-20">
          <button 
            onClick={handlePrev}
            className="w-12 h-12 rounded-full bg-white border border-slate-200 shadow-sm flex items-center justify-center text-slate-500 active:scale-95"
            aria-label="Previous Subject"
          >
            <ChevronUp className="w-6 h-6" />
          </button>
