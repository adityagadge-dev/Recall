    ...
    <motion.div id="leaderboard-page" initial="hidden" animate="visible" variants={{ hidden: { opacity: 0 }, visible: { opacity: 1, transition: { staggerChildren: 0.1 } } }} className="space-y-8 pb-12">
      ...
      <motion.div variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 border-b border-[#323B4E] pb-6">
        <div>
          ...
          <h1 className="text-2xl sm:text-3xl font-extrabold text-[#F7F8FC]">Global Mastery Leaderboard</h1>
        ...
        <div className="flex items-center gap-1.5 rounded-xl border border-[#323B4E] bg-[#0D1017] p-1 text-xs">
          <button
            ...
            className={`rounded-lg px-3 py-1 text-xs font-bold transition ${
              timeframe === 'weekly' ? 'bg-teal-600 text-white shadow' : 'text-[#9AA4B8] hover:bg-[#1A2030]'
            }`}
          >
            This Week
          </button>
          <button
            ...
            className={`rounded-lg px-3 py-1 text-xs font-bold transition ${
              timeframe === 'all_time' ? 'bg-teal-600 text-white shadow' : 'text-[#9AA4B8] hover:bg-[#1A2030]'
            }`}
          >
            All-Time
          </button>
          ...
          <div
            ...
            className={`relative rounded-3xl border p-6 text-center space-y-3 transition-transform duration-300 hover:-translate-y-1 ${
              idx === 0
                ...
                : idx === 1 ? 'border-[#323B4E] bg-[#0D1017] shadow-sm' : 'border-orange-100 bg-orange-50 shadow-sm'
            }`}
          >
            <div className="mx-auto relative w-16 h-16">
              <img
                src={entry.avatarUrl}
                alt={entry.userName}
                className="w-full h-full rounded-full object-cover border-2 border-[#323B4E]"
              />
            ...
            <div>
              <h4 className="text-sm font-bold text-[#F7F8FC]">{entry.userName}</h4>
              <span className="text-xs text-[#9AA4B8] font-mono">Level {entry.level} Practitioner</span>
            </div>
      ...
      <motion.div variants={{ hidden: { opacity: 0, y: 15 }, visible: { opacity: 1, y: 0 } }} className="rounded-3xl border border-[#323B4E] bg-[#11151F] p-4 sm:p-6 space-y-3">
        <h3 className="text-xs font-bold uppercase tracking-wider text-[#9AA4B8] px-2">Complete League Standings</h3>
...
export const ProfilePage: React.FC = () => {
  ...
  return (
    <div id="profile-page" className="max-w-4xl mx-auto space-y-8 pb-12">
      {/* Profile Header */}
      <div className="rounded-3xl border border-[#323B4E] bg-[#11151F] p-6 sm:p-8">
        <div className="flex flex-col sm:flex-row items-center gap-6">
          ...
          <div className="space-y-2 text-center sm:text-left flex-1">
            ...
            <p className="text-xs text-[#9AA4B8]">{user?.email || 'aria@recall.edu'} • Joined March 2026</p>
      ...
      <div className="rounded-3xl border border-[#323B4E] bg-[#11151F] p-6 sm:p-8 space-y-6">
        <h3 className="text-base font-bold text-[#F7F8FC]">Learning Environment Preferences</h3>

        <div className="space-y-4">
          <div className="flex items-center justify-between p-3 rounded-2xl border border-[#323B4E] bg-[#0D1017]">
            <div className="flex items-center gap-3">
              <Mic className="h-5 w-5 text-[#0F766E]" />
              <div>
                <h5 className="text-xs font-bold text-[#F7F8FC]">Voice Microphone Input</h5>
                <p className="text-[11px] text-[#9AA4B8]">Enable voice recording for Teach-Back Feynman exercises</p>
              </div>
            </div>
            <button
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl border border-[#323B4E] bg-[#0D1017]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h5 className="text-xs font-bold text-[#F7F8FC]">Kinetic Sound & Haptics</h5>
                <p className="text-[11px] text-[#9AA4B8]">Play auditory cues on XP gains, wheel clicks, and level-ups</p>
              </div>
            </div>
            <button
          ...
          <div className="flex items-center justify-between p-3 rounded-2xl border border-[#323B4E] bg-[#0D1017]">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h5 className="text-xs font-bold text-[#F7F8FC]">Spaced Retrieval Reminders</h5>
                <p className="text-[11px] text-[#9AA4B8]">Daily notification to maintain your active streak</p>
              </div>
            </div>
            <button
