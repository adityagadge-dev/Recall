    ...
    <div className={`w-full ${className}`}>
      {showLabels && (
        <div className="mb-1.5 flex items-center justify-between text-xs">
          <div className="flex items-center gap-2 font-semibold text-[#F7F8FC]">
          ...
          <div className="flex items-center gap-1 font-mono text-[11px] text-[#9AA4B8]">
      ...
      <div className="relative h-2.5 w-full overflow-hidden rounded-full bg-slate-200 border border-[#4A5568]/50">
        <motion.div
          initial={{ width: 0 }}
        ...
        >
          <div className="absolute top-0 right-0 bottom-0 w-2 bg-[#11151F]/40 blur-[1px]" />
        </motion.div>
      </div>
    </div>
  );
  ...
  return (
    <div className="flex flex-col items-center justify-center">
      <div className="relative flex items-center justify-center" style={{ width: size, height: size }}>
        ...
        <span className="absolute text-[11px] font-bold font-mono text-[#F7F8FC]">
          {percentage}%
        </span>
      </div>
      {label && <span className="mt-1 text-[11px] font-medium text-[#9AA4B8]">{label}</span>}
    </div>
  );
};
