...
export const QuestionCard: React.FC<QuestionCardProps> = ({
  ...
  return (
    <div id={`question-card-${question.id}`} className="w-full max-w-3xl mx-auto rounded-3xl border border-[#323B4E] bg-[#11151F] p-6 sm:p-8 shadow-2xl">
      {/* Progress Header */}
      <div className="mb-6 flex items-center justify-between border-b border-[#323B4E] pb-4">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs font-semibold text-[#9AA4B8] capitalize">
      ...
      {/* Question Prompt */}
      <h3 className="text-lg sm:text-xl font-bold text-[#F7F8FC] mb-6 leading-snug">
        {question.prompt}
      </h3>
      ...
      {question.options && question.options.length > 0 && (
        <div className="space-y-3 mb-6">
          {question.options.map((opt) => {
            ...
            let optStyle = 'border-[#323B4E] bg-[#0D1017] hover:border-slate-600 hover:bg-[#0D1017] text-[#F7F8FC]';

            if (isSelected && !submitted) {
              optStyle = 'border-[#0F766E] bg-lime-950/30 text-[#F7F8FC] ring-1 ring-[#0F766E]';
            }
            ...
            if (submitted) {
              if (opt.isCorrect) {
                ...
                optStyle = 'border-[#323B4E] bg-[#11151F] opacity-50 text-[#9AA4B8]';
              }
            }
      ...
      {question.type === 'short_answer' && (
        <div className="mb-6">
          <textarea
            ...
            className="w-full rounded-xl border border-[#323B4E] bg-[#11151F] p-3.5 text-xs text-[#F7F8FC] placeholder-slate-500 focus:border-[#0F766E] focus:outline-none"
          />
        </div>
      )}
      ...
      {submitted && showInstantFeedback && (
        ...
        >
          ...
          <p className="text-xs text-[#9AA4B8] leading-relaxed mt-1">{question.explanation}</p>
        </motion.div>
      )}

      {/* Controls / Hint / Submit */}
      <div className="flex items-center justify-between pt-2 border-t border-[#323B4E]">
        <div>
          {question.hint && (
            <button
              onClick={() => setShowHint(!showHint)}
              className="flex items-center gap-1 text-xs text-[#9AA4B8] hover:text-teal-300 transition"
            >
        ...
        <div>
          {!submitted ? (
            ...
            <span className="text-xs font-mono text-[#9AA4B8]">Response Recorded</span>
          )}
        </div>
      </div>
    </div>
