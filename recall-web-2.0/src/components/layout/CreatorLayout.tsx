...
export const CreatorLayout: React.FC = () => {
  ...
  return (
    <div id="creator-portal-root" className="min-h-screen bg-[#07080C] text-[#F7F8FC] flex flex-col md:flex-row font-sans selection:bg-[#8B72C9] selection:text-white">
      {/* Desktop Sidebar */}
      <aside className="hidden md:flex flex-col w-64 shrink-0 border-r border-[#1A2030] bg-[#0D1017] p-4 justify-between h-screen sticky top-0 shadow-sm z-40">
        <div>
          <Link to="/" className="flex items-center px-2 py-3 mb-6">
            <RecallLogo color="#8B72C9" />
          </Link>
          
          <div className="mb-6 rounded-2xl border border-[#323B4E] bg-[#07080C] p-4 shadow-sm relative overflow-hidden group hover:shadow-md transition-shadow">
            ...
            <div className="relative z-10 flex items-center justify-between">
              <span className="text-sm font-bold text-[#F7F8FC]">Creator Studio</span>
            ...
            <div className="mt-2 text-[11px] text-[#9AA4B8] font-medium">Manage research & curriculum</div>
          </div>
        ...
        <div className="pt-4 border-t border-[#1A2030] space-y-2">
          <Link
            to="/creator/profile"
            className="flex items-center gap-3 rounded-xl p-2 transition hover:bg-[#07080C] border border-transparent hover:border-[#323B4E]"
          >
            <img
            ...
            <div className="min-w-0 flex-1">
              <p className="text-sm font-bold text-[#F7F8FC] truncate">{user?.name || 'Dr. Elena Rostova'}</p>
              <p className="text-xs text-[#9AA4B8] truncate">Senior Creator</p>
            </div>
          </Link>
          <button
            onClick={signOut}
            className="flex w-full items-center gap-2 rounded-xl px-3 py-2 text-xs font-bold text-[#9AA4B8] transition hover:bg-[#60479C]/10 hover:text-[#60479C]"
          >
      ...
      <div className="flex-1 flex flex-col min-w-0 pb-16 md:pb-0 h-screen overflow-y-auto">
        <header className="sticky top-0 z-30 flex h-16 items-center justify-between border-b border-[#1A2030] bg-[#07080C]/80 px-4 sm:px-6 lg:px-8 backdrop-blur-md">
