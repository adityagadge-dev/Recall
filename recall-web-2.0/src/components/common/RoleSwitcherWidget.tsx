  ...
  return (
    <div id="role-switcher-widget" className="fixed bottom-4 right-4 z-50">
      {isOpen && (
        <div className="mb-3 w-80 rounded-xl border border-[#262f3f] bg-[#0f131a]/95 p-4 shadow-2xl backdrop-blur-md">
          <div className="mb-3 flex items-center justify-between border-b border-[#202735] pb-2">
            <div>
              <span className="text-xs font-semibold uppercase tracking-wider text-[#687286]">Dev Role Simulator</span>
              <p className="text-[11px] text-[#9AA4B8]">Clerk / Role-Aware Integration Layer</p>
            </div>
              ...
              return (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="line-clamp-1 text-[10px] text-[#687286]">{r.desc}</p>
                  </div>
                </button>
              );
            })}
          ...
          <div className="mt-3 border-t border-[#202735] pt-2 text-[10px] text-[#687286] flex items-center justify-between">
            <span>User: <strong className="text-slate-200">{user?.name || 'Guest'}</strong></span>
            <span className="text-[#9AA4B8]">Clerk ready</span>
          </div>
        </div>
      )}
      ...
      >
        ...
        {isOpen ? <ChevronDown className="h-3.5 w-3.5 text-[#687286]" /> : <ChevronUp className="h-3.5 w-3.5 text-[#687286]" />}
      </button>
    </div>
  );
};
