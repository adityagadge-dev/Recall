          ...
          >
            ...
            <div>
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[10px] uppercase font-bold tracking-wider text-[#687286]">Awarded</span>
              </div>
              <p className="text-xs text-[#687286] line-clamp-1">{item.reason}</p>
            </div>
          </motion.div>
        ))}
      </AnimatePresence>
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
      ...
      >
        <button
          ...
          className="absolute top-4 right-4 text-[#687286] hover:text-white transition"
        >
          <X className="h-5 w-5" />
        </button>
        ...
        <p className="text-sm text-[#687286] mb-4">{unlockedBadge.description}</p>

        <div className="rounded-xl border border-[#232b3c] bg-[#171c28] p-3 text-left mb-5">
          <div className="flex items-center justify-between text-xs text-[#687286] mb-1">
