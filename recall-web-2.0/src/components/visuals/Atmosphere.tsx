...
function buildNoiseTile() {
  ...
  for (let y = 0; y < 8; y++) {
    for (let x = 0; x < 8; x++) {
      if (rand() > 0.72) {
        rects += `<rect x="${x}" y="${y}" width="1" height="1" fill="#ffffff" opacity="${(
          rand() * 0.5 +
          0.15
        ).toFixed(2)}"/>`;
      }
...
export function CursorSpotlight() {
  ...
  return (
    <div
      ...
      style={{
        background:
          "radial-gradient(circle, rgba(255,107,97,0.12) 0%, rgba(255,107,97,0.04) 35%, transparent 70%)",
      }}
      aria-hidden="true"
    />
  );
...
export function PixelAtmosphere({
  ...
  return (
    ...
    >
      {!isGlobal && (
        <div className="absolute inset-0 bg-gradient-to-tr from-[#11151F]/40 to-[#07080C]/40 mix-blend-overlay" />
      )}
